...
def markdown(text):
    ...
    return mark_safe(markdown.markdown(text, extensions=["smarty", "nl2br"],))
...
@register.filter
def surround_cyrillic(value, span_class="cyrillic-char"):
    from ..utils import is_roman_chars
    ...
    for c in value:
        if is_roman_chars(c):
            ...
            result += '<span class="%s">%s</span>' % (span_class, c)
